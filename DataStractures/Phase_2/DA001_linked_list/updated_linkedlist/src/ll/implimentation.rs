...
impl<T: std::fmt::Debug> GenericSinglyLinkedList<T> {
    ...
    pub fn remove_nth_from_end(&mut self, index: usize) {
        let mut crn = &mut self.head;
    ...
    pub fn detect_cycle(&self) -> bool {
        ...
        while let Some(fast_node) = fast {
            if let Some(next_node) = fast_node.next.as_ref() {
                if let Some(slow_node) = slow {
                    fast = next_node.next.as_ref(); // 2x Speed
                    slow = slow_node.next.as_ref(); // 1x speed

                    if let (Some(slow_moving_node), Some(fast_moving_node)) = (slow, fast)
                        && std::ptr::eq(slow_moving_node, fast_moving_node)
                    {
                        return true;
                    }
                }
